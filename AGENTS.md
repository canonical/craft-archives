...
`canonical/craft-archives` is a Python library for managing interaction with software
package repositories (for example, apt sources and keyrings) on behalf of tools that use
the [Craft Parts](https://github.com/canonical/craft-parts) library.

## Craft apps and libraries

Craft Archives is used by craft apps, including but not limited to Charmcraft,
...
Craft apps use Craft Archives in conjunction with the following craft libraries:
...
Craft Archives uses [uv](https://docs.astral.sh/uv/) for dependency management.
